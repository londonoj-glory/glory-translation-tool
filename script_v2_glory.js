...
════════════════════════════════════════════════════
    ...
    async callGloryTranslationAPI(text, sourceLang, targetLang) {
        // Sistema de traducción empresarial Glory completamente reescrito
        return new Promise((resolve) => {
            setTimeout(() => {
                // Frases completas comunes empresariales y cotidianas
                const completePhrasesTranslations = {
                    'en-es': {
                        ...
                        // Frases de tiempo y acciones
                        'a long time ago': 'hace mucho tiempo',
                        'for many years': 'por muchos años',
                        'i have been': 'he estado',
                        'we have done': 'hemos hecho',
                        'it has been': 'ha sido',
                        'i would like': 'me gustaría',
                        'we should do': 'deberíamos hacer',
                ...
                // Diccionario robusto de palabras individuales
                const robustDictionary = {
                    'en-es': {
                        // Pronombres y auxiliares
                        'i': 'yo', 'you': 'tú', 'he': 'él', 'she': 'ella', 'it': 'eso', 'we': 'nosotros', 'they': 'ellos',
                        'am': 'soy', 'are': 'eres', 'is': 'es', 'was': 'era', 'were': 'eran', 'be': 'ser', 'been': 'sido',
                        'have': 'tengo', 'has': 'tiene', 'had': 'tenía', 'do': 'hacer', 'does': 'hace', 'did': 'hizo',
                        'will': 'voy a', 'would': 'haría', 'can': 'puedo', 'could': 'podría', 'should': 'debería',
                        
                        // Artículos y conectores
                        'the': 'el', 'a': 'un', 'an': 'un', 'and': 'y', 'or': 'o', 'but': 'pero', 'if': 'si', 'because': 'porque',
                        'to': 'a', 'for': 'para', 'of': 'de', 'in': 'en', 'on': 'en', 'at': 'en', 'by': 'por', 'with': 'con',
                        'from': 'desde', 'up': 'arriba', 'down': 'abajo', 'out': 'fuera', 'off': 'apagado', 'over': 'sobre',
                        'under': 'bajo', 'again': 'otra vez', 'further': 'más', 'then': 'entonces', 'than': 'que',
                        
                        // Palabras interrogativas y temporales
                        'what': 'qué', 'when': 'cuándo', 'where': 'dónde', 'who': 'quién', 'why': 'por qué', 'how': 'cómo',
                        'now': 'ahora', 'today': 'hoy', 'tomorrow': 'mañana', 'yesterday': 'ayer', 'always': 'siempre',
                        'never': 'nunca', 'sometimes': 'a veces', 'often': 'frecuentemente', 'usually': 'usualmente',
                        
                        // Adjetivos comunes
                        'good': 'bueno', 'bad': 'malo', 'big': 'grande', 'small': 'pequeño', 'new': 'nuevo', 'old': 'viejo',
                        'young': 'joven', 'happy': 'feliz', 'sad': 'triste', 'easy': 'fácil', 'difficult': 'difícil',
                        'important': 'importante', 'interesting': 'interesante', 'beautiful': 'hermoso', 'nice': 'agradable',
                        
                        // Verbos comunes
                        'go': 'ir', 'come': 'venir', 'see': 'ver', 'know': 'saber', 'think': 'pensar', 'want': 'querer',
                        'need': 'necesitar', 'like': 'gustar', 'love': 'amar', 'work': 'trabajar', 'live': 'vivir',
                        'say': 'decir', 'tell': 'contar', 'ask': 'preguntar', 'give': 'dar', 'take': 'tomar',
                        'make': 'hacer', 'get': 'obtener', 'help': 'ayudar', 'find': 'encontrar', 'use': 'usar',
                        
                        // Sustantivos básicos
                        'time': 'tiempo', 'day': 'día', 'year': 'año', 'week': 'semana', 'month': 'mes',
                        'house': 'casa', 'home': 'hogar', 'family': 'familia', 'friend': 'amigo', 'person': 'persona',
                        'people': 'gente', 'man': 'hombre', 'woman': 'mujer', 'child': 'niño', 'children': 'niños',
                        'work': 'trabajo', 'job': 'empleo', 'money': 'dinero', 'food': 'comida', 'water': 'agua',
                        
                        // Números básicos
                        'one': 'uno', 'two': 'dos', 'three': 'tres', 'four': 'cuatro', 'five': 'cinco',
                        'six': 'seis', 'seven': 'siete', 'eight': 'ocho', 'nine': 'nueve', 'ten': 'diez',
                        
                        // Palabras del contexto específico reportado
                        'around': 'alrededor', 'another': 'otro', 'stumbling': 'tropezando', 'block': 'bloque',
                        'kindness': 'bondad', 'act': 'acto', 'cold': 'frío', 'brunch': 'brunch', 
                        'done': 'hecho', 'walk': 'caminar', 'red': 'rojo', 'carpet': 'alfombra',
                        'anxious': 'ansioso', 'mean': 'significa', 'helped': 'ayudé', 'much': 'mucho',
                        'against': 'contra', 'years': 'años', 'name': 'nombre', 'guess': 'supongo',
                        'saying': 'diciendo', 'satisfied': 'satisfecho', 'perfectly': 'perfectamente',
                        'fell': 'cayó', 'stumble': 'tropiezo', 'translated': 'traducido'
                    },
                    'es-en': {
                        'yo': 'I', 'tú': 'you', 'él': 'he', 'ella': 'she', 'nosotros': 'we', 'ellos': 'they',
                        'soy': 'am', 'eres': 'are', 'es': 'is', 'era': 'was', 'somos': 'are', 'son': 'are',
                        'el': 'the', 'la': 'the', 'un': 'a', 'una': 'a', 'y': 'and', 'o': 'or', 'pero': 'but',
                        'qué': 'what', 'cuándo': 'when', 'dónde': 'where', 'quién': 'who', 'por qué': 'why', 'cómo': 'how',
                        'bueno': 'good', 'malo': 'bad', 'grande': 'big', 'pequeño': 'small', 'nuevo': 'new', 'viejo': 'old'
                    }
                };

                const langPair = `${sourceLang}-${targetLang}`;
                
                // 1. Intentar traducir como frase completa primero
                const completeDict = completePhrasesTranslations[langPair] || {};
                const lowerText = text.toLowerCase().trim();
                
                if (completeDict[lowerText]) {
                    resolve(completeDict[lowerText]);
                    return;
                }
                
                // 2. Intentar traducir usando el diccionario robusto
                const wordDict = robustDictionary[langPair] || {};
                let translation = this.translateWithConsistency(text, wordDict, sourceLang, targetLang);
    ...
    translateWithConsistency(text, dict, sourceLang, targetLang) {
        // Sistema inteligente que mantiene consistencia de idioma
        const words = text.toLowerCase().split(/\s+/);
        let translatedWords = [];
        let translationSuccess = 0;
        
        // Primera pasada: intentar traducir cada palabra
        ...
        const translationRate = translationSuccess / words.length;
        
        if (translationRate >= 0.6) {
            // Si traducimos 60% o más, usar la traducción
            return translatedWords.join(' ');
        } else if (sourceLang === 'en' && targetLang === 'es') {
            // Si no podemos traducir bien del inglés, dar una traducción básica
            return `Traducción aproximada: ${text}`;
        } else {
            // Para otros casos, mantener el texto original
            return text;
        }
    }

    generateGloryTranslation(word, sourceLang, targetLang) {
        // Diccionario extendido de palabras comunes Glory
        const extendedDictionary = {
            'en-es': {
                // Palabras básicas más comunes
                'i': 'yo', 'you': 'tú', 'he': 'él', 'she': 'ella', 'it': 'eso', 'we': 'nosotros', 'they': 'ellos',
                'am': 'soy', 'are': 'eres', 'is': 'es', 'was': 'era', 'were': 'eran', 'be': 'ser', 'been': 'sido',
                'have': 'tener', 'has': 'tiene', 'had': 'tenía', 'do': 'hacer', 'does': 'hace', 'did': 'hizo',
                'will': 'será', 'would': 'sería', 'can': 'puede', 'could': 'podría', 'should': 'debería',
                'the': 'el', 'a': 'un', 'an': 'un', 'and': 'y', 'or': 'o', 'but': 'pero', 'if': 'si',
                'to': 'a', 'for': 'para', 'of': 'de', 'in': 'en', 'on': 'en', 'at': 'en', 'by': 'por',
                'with': 'con', 'from': 'de', 'up': 'arriba', 'down': 'abajo', 'out': 'fuera', 'off': 'apagado',
                'over': 'sobre', 'under': 'bajo', 'again': 'otra vez', 'further': 'más', 'then': 'entonces',
                'once': 'una vez', 'here': 'aquí', 'there': 'allí', 'when': 'cuando', 'where': 'donde',
                'why': 'por qué', 'how': 'cómo', 'all': 'todo', 'any': 'cualquier', 'both': 'ambos',
                'each': 'cada', 'few': 'pocos', 'more': 'más', 'most': 'mayoría', 'other': 'otro',
                'some': 'algunos', 'such': 'tal', 'no': 'no', 'nor': 'ni', 'not': 'no', 'only': 'solo',
                'own': 'propio', 'same': 'mismo', 'so': 'tan', 'than': 'que', 'too': 'también',
                'very': 'muy', 'just': 'solo', 'now': 'ahora', 'good': 'bueno', 'bad': 'malo',
                'new': 'nuevo', 'old': 'viejo', 'first': 'primero', 'last': 'último', 'long': 'largo',
                'great': 'genial', 'little': 'pequeño', 'right': 'correcto', 'big': 'grande', 'high': 'alto',
                'different': 'diferente', 'small': 'pequeño', 'large': 'grande', 'next': 'siguiente',
                'early': 'temprano', 'young': 'joven', 'important': 'importante', 'public': 'público',
                'able': 'capaz', 'around': 'alrededor', 'held': 'mantuvo', 'grudge': 'rencor', 
                'against': 'contra', 'years': 'años', 'mention': 'mención', 'name': 'nombre', 
                'mood': 'estado de ánimo', 'ago': 'hace', 'know': 'saber', 'what': 'qué', 
                'time': 'tiempo', 'guess': 'supongo', 'saying': 'diciendo', 'mean': 'significa',
                'me': 'mí', 'that': 'que', 'her': 'ella', 'year': 'año'
            },
            'es-en': {
                // Traducción inversa español-inglés
                'yo': 'I', 'tú': 'you', 'él': 'he', 'ella': 'she', 'eso': 'it', 'nosotros': 'we', 'ellos': 'they',
                'soy': 'am', 'eres': 'are', 'es': 'is', 'era': 'was', 'eran': 'were', 'ser': 'be', 'sido': 'been',
                'tener': 'have', 'tiene': 'has', 'tenía': 'had', 'hacer': 'do', 'hace': 'does', 'hizo': 'did',
                'será': 'will', 'sería': 'would', 'puede': 'can', 'podría': 'could', 'debería': 'should',
                'el': 'the', 'un': 'a', 'y': 'and', 'o': 'or', 'pero': 'but', 'si': 'if',
                'a': 'to', 'para': 'for', 'de': 'of', 'en': 'in', 'por': 'by', 'con': 'with'
            }
        };

        const langPair = `${sourceLang}-${targetLang}`;
        const extDict = extendedDictionary[langPair] || {};
        
        // Buscar en diccionario extendido primero
        if (extDict[word.toLowerCase()]) {
            return extDict[word.toLowerCase()];
        }

        // Transformaciones morfológicas Glory mejoradas
        const gloryTransformations = {
            'es-en': (word) => {
                ...
                return word; // Mantener palabra original sin prefijos problemáticos
            },
            'en-es': (word) => {
        ...
        // Si no se puede traducir, mantener la palabra original
        // ELIMINAMOS los prefijos problemáticos [ES], [EN], etc.
        return word;
    }
