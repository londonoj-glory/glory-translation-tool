...
class AdvancedAudioTranslationTool {
    ...
    initializeElements() {
        this.elements = {
            ...
            resumeAudioBtn: document.getElementById('resume-audio-btn'),
            stopAudioBtn: document.getElementById('stop-audio-btn'),
            voiceVariantSelect: document.getElementById('voice-variant-select'),
    ...
    initializeEventListeners() {
        ...
        this.elements.resumeAudioBtn.addEventListener('click', () => this.resumeAudio());
        this.elements.stopAudioBtn.addEventListener('click', () => this.stopAudio());
        this.elements.voiceVariantSelect.addEventListener('change', () => this.updateVoiceVariant());
    ...
    initRecognition() {
        ...
        // Configuración optimizada para mejor captura de audio
        this.state.recognition.continuous = true;
        this.state.recognition.interimResults = true;
        this.state.recognition.maxAlternatives = 3; // Más alternativas para mejor precisión
        
        // Configuraciones específicas para WebKit (Chrome/Safari)
        if ('webkitSpeechRecognition' in window) {
            this.state.recognition.webkitContinuous = true;
            this.state.recognition.webkitInterimResults = true;
        }
        
        // Add retry counter for long recordings
        this.state.recognitionRetries = 0;
        this.state.maxRetries = 10; // Max retries for long recordings
        ...
        this.state.recognition.onerror = (event) => {
            ...
            switch (event.error) {
                case 'network':
                    ...
                    break;
                case 'aborted':
                    // Don't restart on aborted - user stopped it
                    break;
                case 'no-speech':
                    if (this.state.isRecording) {
                        this.updateStatusText('🎤 Esperando tu voz... Habla más cerca del micrófono');
                        // Dar más tiempo antes de reiniciar para evitar interrupciones
                        setTimeout(() => {
                            if (this.state.isRecording) {
                ...
                case 'not-allowed':
                    ...
                    break;
                default:
                    this.showToast(`Error de reconocimiento: ${event.error}`, 'error');
                    if (this.state.isRecording) {
                        this.restartRecognitionIfNeeded();
                    }
            }
        ...
        this.state.recognition.onend = () => {
            if (this.state.isRecording) {
                ...
                if (selectedTime === 0 || (this.state.timeLeft && this.state.timeLeft > 0)) {
                    // Add small delay before restart to avoid rapid restart loops
                    setTimeout(() => {
                        if (this.state.isRecording && this.state.recognitionRetries < this.state.maxRetries) {
                            this.restartRecognitionIfNeeded();
                        } else if (this.state.recognitionRetries >= this.state.maxRetries) {
                            this.showToast('Límite de reintentos alcanzado. Grabación finalizada.', 'warning');
                            this.finishRecording();
                        }
                    }, 200);
                } else {
        ...
        this.state.recognition.onstart = () => {
            this.updateStatusText('Reconocimiento de voz iniciado');
        };
    }
    ...
    restartRecognitionIfNeeded() {
        ...
        if (this.state.recognitionRetries > this.state.maxRetries) {
            this.showToast('Demasiados reintentos. Finalizando grabación.', 'warning');
            this.finishRecording();
            return;
        }
        ...
        try {
            ...
            // Add delay before restart to avoid conflicts
            setTimeout(() => {
                if (this.state.isRecording) {
                    try {
                        this.state.recognition.lang = langConfig.source;
                        this.state.recognition.start();
                        this.updateStatusText(`Reconocimiento reiniciado... (intento ${this.state.recognitionRetries})`);
                    } catch (error) {
    ...
    async startRecording() {
        try {
            ...
            await this.showRecordingCountdown();
            ...
            // Esperar un momento para que el micrófono se estabilice
            await this.delay(500);
            
            this.startSpeechRecognition();
            this.startTimer();
            
            this.showToast('¡Grabación iniciada! Habla ahora', 'success');
            this.updateStatusText('🎤 Grabando... Habla claramente');
            
            // Mostrar consejo aleatorio después de 3 segundos
            setTimeout(() => {
                if (this.state.isRecording) {
    ...
    async showRecordingCountdown() {
        const countdownTexts = ['Preparándose...', '3', '2', '1', '¡Habla ahora!'];
        const statusElement = this.elements.statusText;
        const originalText = statusElement.textContent;
        
        for (let i = 0; i < countdownTexts.length; i++) {
            statusElement.textContent = countdownTexts[i];
            statusElement.style.fontSize = i > 0 && i < 4 ? '1.5em' : '1em';
            statusElement.style.fontWeight = i > 0 && i < 4 ? 'bold' : 'normal';
            statusElement.style.color = i === 4 ? '#4CAF50' : '';
            
            await this.delay(i === 0 ? 1000 : i === 4 ? 500 : 800);
        }
    ...
    showRecordingTips() {
        ...
        this.showToast(randomTip, 'info', 4000);
    }
    ...
    finishRecording() {
        ...
        const text = (this.state.finalTranscript + this.state.interimTranscript).trim();
        if (text) {
            this.translateAndAdd(text);
            this.updateStatusText('✅ Grabación completada y procesada');
        } else {
            this.showToast('❌ No se detectó audio claro. Revisa los consejos de grabación.', 'warning');
            this.showRecordingTips();
            this.updateStatusText('❌ No se detectó audio - Intenta nuevamente');
        }
    ...
    startSpeechRecognition() {
        ...
        // Configuraciones optimizadas para mejor captura
        this.state.recognition.lang = langConfig.source;
        this.state.recognition.continuous = true;
        this.state.recognition.interimResults = true;
        this.state.recognition.maxAlternatives = 3;
        
        // Configuraciones adicionales para mejor sensibilidad
        try {
            // Estas propiedades no están disponibles en todos los navegadores
            if ('webkitSpeechRecognition' in window) {
                this.state.recognition.webkitContinuous = true;
                this.state.recognition.webkitInterimResults = true;
            }
        } catch (e) {
            console.log('Algunas configuraciones avanzadas no están disponibles');
        }
        
        try {
            this.state.recognition.start();
            this.updateStatusText('🎤 Reconocimiento activado - Esperando voz...');
        } catch (error) {
            console.error('Error al iniciar reconocimiento:', error);
            this.showToast('Error al activar el reconocimiento de voz', 'error');
        }
    }
    ...
    updateButtonStates() {
        ...
        this.elements.playTranslationBtn.disabled = !hasHistory;
    ...
    updateLanguageDisplay() {
        ...
        // Update voice variants for the new language
        this.populateVoiceVariants();
    }
    ...
    updateTimeDisplay() {
        ...
        if (selectedTime >= 120) {
            this.showToast('Grabaciones largas pueden requerir mejor conexión. El reconocimiento se reiniciará automáticamente si es necesario.', 'warning');
        } else if (selectedTime >= 60) {
            this.showToast('Grabaciones de 1+ minutos pueden reiniciarse automáticamente por limitaciones del navegador', 'info');
        }
    }
    ...
    // Text-to-Speech (Enhanced with Pause/Resume)
    speakText(text, lang) {
        // Stop any current audio
        this.stopAudio();
        
        // Store current text and language for pause/resume functionality
        ...
        this.state.textChunks = this.splitTextIntoChunks(text);
        
        // Start playing
        this.playTextFromChunk(0);
    }
    ...
    playTextFromChunk(chunkIndex) {
        if (chunkIndex >= this.state.textChunks.length) {
            this.onAudioEnd();
            return;
        }

        const chunk = this.state.textChunks[chunkIndex].trim();
        if (!chunk) {
            this.playTextFromChunk(chunkIndex + 1);
            return;
        }
        ...
        const selectedVoice = this.getSelectedVoice(this.state.currentLang);
    ...
    getSelectedVoice(lang) {
        const selectedVariant = this.elements.voiceVariantSelect.value;
        
        if (selectedVariant === 'auto') {
            // Use the original voice selection logic
            ...
            // Use specifically selected voice
            return this.state.voices.find(voice => voice.name === selectedVariant) || 
                   this.state.voices.find(voice => voice.lang === lang);
        }
    }
    ...
    pauseAudio() {
        if (this.state.audioState === 'playing') {
            ...
            this.updateStatusText('Audio pausado');
            this.updateAudioControlButtons();
    ...
    resumeAudio() {
        if (this.state.audioState === 'paused') {
            ...
            this.updateStatusText('Reproduciendo...');
            this.updateAudioControlButtons();
            this.showToast('Audio reanudado', 'success');
        }
    }
    ...
    stopAudio() {
        ...
        this.state.currentChunkIndex = 0;
        this.updateStatusText('Audio detenido');
        this.updateAudioControlButtons();
    }
    ...
    updateAudioControlButtons() {
        const { playTranslationBtn, pauseAudioBtn, resumeAudioBtn, stopAudioBtn } = this.elements;
        
        // Reset classes
        [playTranslationBtn, pauseAudioBtn, resumeAudioBtn, stopAudioBtn].forEach(btn => {
            btn.classList.remove('audio-playing', 'audio-paused', 'audio-stopped');
        });

        switch (this.state.audioState) {
            case 'playing':
                playTranslationBtn.style.display = 'none';
                pauseAudioBtn.style.display = 'flex';
                resumeAudioBtn.style.display = 'none';
                stopAudioBtn.style.display = 'flex';
                pauseAudioBtn.classList.add('audio-playing');
                stopAudioBtn.classList.add('audio-playing');
                break;
                
            case 'paused':
                playTranslationBtn.style.display = 'none';
                pauseAudioBtn.style.display = 'none';
                resumeAudioBtn.style.display = 'flex';
                stopAudioBtn.style.display = 'flex';
                resumeAudioBtn.classList.add('audio-paused');
                stopAudioBtn.classList.add('audio-paused');
                break;
                
            case 'stopped':
            default:
                playTranslationBtn.style.display = 'flex';
                pauseAudioBtn.style.display = 'none';
                resumeAudioBtn.style.display = 'none';
                stopAudioBtn.style.display = 'none';
                playTranslationBtn.classList.add('audio-stopped');
                break;
        }
    }
    ...
    populateVoiceVariants() {
        const currentLanguagePair = this.elements.languageSelect.value;
        const langConfig = this.supportedLanguages[currentLanguagePair];
        
        if (!langConfig) return;
        
        const targetLang = langConfig.target;
        
        // Find voices for target language
        const availableVoices = this.state.voices.filter(voice => 
            voice.lang.startsWith(targetLang.split('-')[0])
        );
        
        // Clear current options except auto
        this.elements.voiceVariantSelect.innerHTML = '<option value="auto">Detección automática</option>';
        
        // Group voices by language/region
        const voiceGroups = this.groupVoicesByRegion(availableVoices, targetLang);
        
        // Add grouped options
        Object.keys(voiceGroups).forEach(region => {
            const optgroup = document.createElement('optgroup');
            optgroup.label = region;
            
            voiceGroups[region].forEach(voice => {
            ...
            this.elements.voiceVariantSelect.appendChild(optgroup);
        });
    }

    groupVoicesByRegion(voices, targetLang) {
        const groups = {};
        
        voices.forEach(voice => {
            let regionName = this.getRegionName(voice.lang);
            
            if (!groups[regionName]) {
                groups[regionName] = [];
            }
            
            groups[regionName].push(voice);
        });
    ...
    getRegionName(langCode) {
        ...
        return regionMap[langCode] || `${langCode.toUpperCase()}`;
    }

    updateVoiceVariant() {
        const selectedVoice = this.elements.voiceVariantSelect.value;
        this.showToast(`Variante de voz seleccionada: ${selectedVoice === 'auto' ? 'Automática' : selectedVoice}`, 'info');
    }
